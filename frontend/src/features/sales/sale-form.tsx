...
export function SaleForm({
  ...
  function updateValue(field: SaleFormField, value: string | boolean): void {
    setValues((current) => ({
      ...current,
      [field]: value,
    }));
  ...
  const submitLabel = mode === "create" ? "Crear venta" : "Guardar cambios";

  const submittingLabel =
    mode === "create" ? "Creando venta…" : "Guardando cambios…";
      ...
      <section className="overflow-hidden rounded-[var(--radius-xl)] bg-surface shadow-[var(--shadow-sm)] ring-1 ring-[var(--color-border-soft)]">
        <div className="border-b border-[var(--color-border-soft)] p-5 sm:p-6">
          ...
          <p className="mt-1 text-sm leading-6 text-muted-foreground">
            Cliente opcional: déjelo sin seleccionar para una venta de mostrador.
          </p>
        </div>
        ...
        <div className="grid gap-6 p-5 sm:p-6 lg:grid-cols-2">
          <Field
            ...
            hint={
              mode === "create"
                ? "Busque por nombre, identificación o teléfono. Déjelo vacío para venta de mostrador."
                : "El cliente no puede modificarse una vez creada la venta."
            }
          ...
          >
            {mode === "edit" ? (
              <div className="flex h-12 items-center rounded-[var(--radius-md)] border border-border bg-surface-muted px-4 text-sm font-medium text-foreground">
                {selectedCustomerLabel ?? "Venta de mostrador (sin cliente)"}
              </div>
          ...
          <div className="lg:col-span-2">
            <Field id="sale-notes" label="Notas" error={errors.notes}>
              <Textarea
                id="sale-notes"
                name="notes"
                value={values.notes}
          ...
          >
            ...
            <span>
              ...
              <span className="mt-1 block text-xs leading-5 text-muted-foreground">
                Las ventas inactivas se conservan para trazabilidad, pero se ocultan del flujo habitual.
              </span>
            </span>
          </label>
        </div>
