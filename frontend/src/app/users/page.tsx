...
import { KeyIcon, SearchIcon } from "@/components/icons/app-icons";
...
export default function UsersPage() {
  ...
  useEffect(() => {
    if (authStatus !== "authenticated" || !token || !hasAccess) {
      return;
    }

    const controller = new AbortController();

    getUsers(token, filters, controller.signal)
      .then((data) => {
  ...
  }, [authStatus, filters, hasAccess, logout, router, token]);
  ...
  if (authStatus === "authenticated" && user && !hasAccess) {
    return (
      ...
      >
        <StatePanel
          title="No tiene acceso a usuarios"
  ...
  return (
    <AppShell
      title="Usuarios"
      description="Administre cuentas, roles y acceso al sistema."
      actions={
        hasAccess ? (
          <Button
            type="button"
    ...
    >
      ...
      >
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm leading-6 text-muted-foreground">
                Busque por usuario, nombre, apellido o correo.
              </p>
            </div>
            ...
            >
              <div className="relative min-w-0 flex-1">
                ...
                <Input
                  ...
                  placeholder="Usuario, nombre, apellido o correo"
              ...
              <select
                ...
                onChange={(event) => {
                  const activeState =
                    event.target.value as UserFilters["activeState"];
        ...
        {loadState.status === "success" && loadState.data.results.length === 0 && (
          <div className="p-6">
            <StatePanel
              title={filters.query ? "No se encontraron usuarios" : "No hay usuarios registrados"}
              message={
                filters.query
                  ? "Revise el usuario, nombre, apellido o correo ingresado."
                  : "El catálogo todavía no contiene usuarios para el filtro seleccionado."
              }
              icon={<KeyIcon />}
              action={
                filters.query ? (
          ...
          <>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[820px] border-collapse">
                ...
                <tbody>
                  {loadState.data.results.map((rowUser, index) => (
                    <tr
                      key={rowUser.id}
                      ref={(element) => {
                      ...
                      tabIndex={0}
                      role="link"
                      aria-label={`Abrir usuario ${rowUser.username}`}
                      onClick={() => {
                        openUser(rowUser.id);
                      }}
                      onKeyDown={(event) => {
                    ...
                    >
                      <td className="px-5 py-4 align-top">
                        <p className="font-semibold text-foreground">{rowUser.username}</p>
                      </td>
                      ...
                      <td className="px-5 py-4 align-top text-sm text-muted-foreground">
                        {rowUser.is_staff && "is_staff"}
                        {rowUser.is_staff && rowUser.groups.length > 0 && ", "}
                        {rowUser.groups.map((role) => ROLE_LABELS[role] ?? role).join(", ")}
                        {!rowUser.is_staff && rowUser.groups.length === 0 && "—"}
                      </td>
                      ...
                      <td className="px-5 py-4 align-top">
                        ...
                        >
                          ...
                          {rowUser.is_active ? "Activo" : "Inactivo"}
                        </span>
                      </td>
                    </tr>
                  ))}
