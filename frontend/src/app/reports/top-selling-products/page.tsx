...
import { ArrowLeftIcon, BoxIcon } from "@/components/icons/app-icons";
...
import { getTopSellingProductsReport } from "@/features/inventory/reports/api";
import {
...
type LoadState =
  | { status: "loading" }
  | { status: "success"; data: TopSellingProductsReport }
...
function getLoadErrorMessage(error: unknown): string {
  ...
  return "No fue posible consultar los productos más vendidos.";
}
...
export default function TopSellingProductsReportPage() {
  ...
  useEffect(() => {
    ...
    getTopSellingProductsReport(token, filters, controller.signal)
      .then((data) => {
  ...
  return (
    <AppShell
      ...
      description="Cantidad vendida y total facturado por producto, en colones."
      actions={
                ...
                {loadState.data.results.map((entry) => (
                  ...
                  >
                    <td className="px-5 py-4 align-top text-sm">
                      <Link
                        href={`/inventory/products/${entry.product.id}`}
                        className="font-medium text-primary hover:underline"
                      >
                        <span className="font-mono">{entry.product.standard_code}</span>
                        <span className="ml-2">{entry.product.name}</span>
                      </Link>
                    </td>
