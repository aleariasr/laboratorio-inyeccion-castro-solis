...
import { getTopCustomersReport } from "@/features/inventory/reports/api";
import {
  EMPTY_TOP_CUSTOMERS_FILTERS,
  type TopCustomersFilters,
  type TopCustomersOrdering,
...
type LoadState =
  | { status: "loading" }
  | { status: "success"; data: TopCustomersReport }
...
function getLoadErrorMessage(error: unknown): string {
  ...
  return "No fue posible consultar los clientes con más ventas.";
}
...
export default function TopCustomersReportPage() {
  ...
  const [filters, setFilters] = useState<TopCustomersFilters>(EMPTY_TOP_CUSTOMERS_FILTERS);

  const [loadState, setLoadState] = useState<LoadState>({ status: "loading" });

  const hasReportsAccess = user ? canReadReports(user) : false;

  const hasActiveDateFilters = filters.dateFrom !== "" || filters.dateTo !== "";
  ...
  useEffect(() => {
    ...
    getTopCustomersReport(token, filters, controller.signal)
      .then((data) => {
  ...
  function updateFilters(
    updater: (current: TopCustomersFilters) => TopCustomersFilters,
  ): void {
    ...
    >
      <div className="mb-5 rounded-[var(--radius-xl)] bg-surface p-5 shadow-[var(--shadow-sm)] ring-1 ring-[var(--color-border-soft)] sm:p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
          <div>
            <label
              htmlFor="top-customers-date-from"
          ...
          <div>
            <label
              htmlFor="top-customers-ordering"
              className="mb-1.5 block text-xs font-semibold uppercase tracking-[0.06em] text-muted-foreground"
            >
              Ordenar por
            </label>
            ...
            <select
              id="top-customers-ordering"
              value={filters.ordering}
              onChange={(event) => {
                const ordering = event.target.value as TopCustomersOrdering;

                updateFilters((current) => ({ ...current, ordering, page: 1 }));
              }}
            ...
            >
              ...
              <option value="sale_count">Cantidad de ventas</option>
            </select>
          </div>
      ...
      {loadState.status === "success" && loadState.data.results.length === 0 && (
        <StatePanel
          ...
          message={
            hasActiveDateFilters
              ? "No hay ventas con cliente asociado para el rango de fechas seleccionado."
              : "Todavía no hay ventas con cliente asociado en el sistema."
          }
          icon={<UsersIcon />}
          action={
            hasActiveDateFilters ? (
