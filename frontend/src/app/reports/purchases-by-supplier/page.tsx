...
import { getPurchasesBySupplierReport } from "@/features/inventory/reports/api";
import {
...
type LoadState =
  | { status: "loading" }
  | { status: "success"; data: PurchasesBySupplierReport }
...
function getLoadErrorMessage(error: unknown): string {
  ...
  return "No fue posible consultar las compras por proveedor.";
}
...
export default function PurchasesBySupplierReportPage() {
  ...
  useEffect(() => {
    ...
    getPurchasesBySupplierReport(token, filters, controller.signal)
      .then((data) => {
  ...
  return (
    <AppShell
      ...
      description="Cantidad de compras y subtotal facturado por proveedor, en colones."
    >
      <div className="mb-5 rounded-[var(--radius-xl)] bg-surface p-5 shadow-[var(--shadow-sm)] ring-1 ring-[var(--color-border-soft)] sm:p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
          <div>
            <label
              htmlFor="purchases-by-supplier-date-from"
            ...
            <Input
              id="purchases-by-supplier-date-from"
              type="date"
          ...
          <div>
            <label
              htmlFor="purchases-by-supplier-date-to"
              className="mb-1.5 block text-xs font-semibold uppercase tracking-[0.06em] text-muted-foreground"
            >
              Hasta
            </label>
            ...
            <Input
              id="purchases-by-supplier-date-to"
              type="date"
      ...
      {loadState.status === "success" && loadState.data.results.length === 0 && (
        <StatePanel
          ...
          message={
            hasActiveFilters
              ? "No hay compras confirmadas para el rango de fechas seleccionado."
              : "Todavía no hay compras confirmadas en el sistema."
          }
          icon={<TruckIcon />}
          action={
            hasActiveFilters ? (
                ...
                {loadState.data.results.map((entry) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-4 text-right align-top text-sm font-semibold text-foreground">
                      {formatMoney(entry.invoice_subtotal)} {entry.currency}
                    </td>
                  </tr>
                ))}
              </tbody>
