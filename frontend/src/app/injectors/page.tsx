...
import { DropletIcon, SearchIcon } from "@/components/icons/app-icons";
import { AppShell } from "@/components/layout/app-shell";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { useAuth } from "@/features/auth/auth-context";
import { canReadInjectors, canWriteInjectors } from "@/features/auth/permissions";
import { getInjectors } from "@/features/injectors/api";
import type { Injector, InjectorFilters } from "@/features/injectors/types";
...
type LoadState =
  ...
  | {
      status: "success";
      data: PaginatedResponse<Injector>;
      message: null;
    }
  | {
...
const INITIAL_FILTERS: InjectorFilters = {
  query: "",
  activeState: "active",
  page: 1,
  pageSize: 50,
...
function getLoadErrorMessage(error: unknown): string {
  ...
  return "No fue posible consultar los inyectores.";
}
...
export default function InjectorsPage() {
  ...
  const [filters, setFilters] = useState<InjectorFilters>(INITIAL_FILTERS);
  ...
  useEffect(() => {
    ...
    getInjectors(token, filters, controller.signal)
      .then((data) => {
  ...
  async function handleRetry(): Promise<void> {
    ...
    try {
      const data = await getInjectors(token, filters);
  ...
  function updateFilters(
    updater: (current: InjectorFilters) => InjectorFilters,
  ): void {
    setLoadState({ status: "loading", data: null, message: null });
  ...
  function openInjector(injectorId: number): void {
  ...
  if (authStatus === "authenticated" && user && !hasCustomersAccess) {
    return (
      ...
      >
        <StatePanel
          title="No tiene acceso a inyectores"
  ...
  return (
    <AppShell
      title="Inyectores"
      description="Consulte inyectores registrados por cliente."
      actions={
        hasWriteAccess ? (
          <Button
            type="button"
            onClick={() => {
              router.push("/injectors/new");
            }}
          >
            Nuevo inyector
          </Button>
    ...
    >
      ...
      >
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            >
              ...
              <select
                ...
                onChange={(event) => {
                  const activeState =
                    event.target.value as InjectorFilters["activeState"];
                ...
                aria-label="Filtrar inyectores por estado"
              >
        ...
        {loadState.status === "success" && loadState.data.results.length === 0 && (
          <div className="p-6">
            <StatePanel
              title={filters.query ? "No se encontraron inyectores" : "No hay inyectores registrados"}
              message={
                filters.query
                  ? "Revise el número o intente con el nombre del cliente."
                  : "El catálogo todavía no contiene inyectores para el filtro seleccionado."
              }
              icon={<DropletIcon />}
              action={
                filters.query ? (
          ...
          <>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[720px] border-collapse">
                ...
                <tbody>
                  {loadState.data.results.map((injector, index) => (
                    <tr
                      key={injector.id}
                      ref={(element) => {
                      ...
                      tabIndex={0}
                      role="link"
                      aria-label={`Abrir inyector ${injector.injector_number}`}
                      onClick={() => {
                        openInjector(injector.id);
                      }}
                      onKeyDown={(event) => {
                    ...
                    >
                      <td className="px-5 py-4 align-top">
                        <p className="font-mono font-semibold text-foreground">{injector.injector_number}</p>
                      </td>
