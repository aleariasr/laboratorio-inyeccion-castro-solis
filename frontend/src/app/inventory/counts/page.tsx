...
import { ClipboardCheckIcon, SearchIcon } from "@/components/icons/app-icons";
import { AppShell } from "@/components/layout/app-shell";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { useAuth } from "@/features/auth/auth-context";
import { canReadInventory, canWriteInventory } from "@/features/auth/permissions";
import { getInventoryCounts } from "@/features/inventory/counts/api";
import type {
  InventoryCount,
...
type LoadState =
  ...
  | {
      status: "success";
      data: PaginatedResponse<InventoryCount>;
      message: null;
    }
  | {
...
const INITIAL_FILTERS: InventoryCountFilters = {
  query: "",
  status: "",
  dateFrom: "",
  dateTo: "",
...
function getLoadErrorMessage(error: unknown): string {
  ...
  return "No fue posible consultar los conteos.";
}
...
export default function InventoryCountsPage() {
  ...
  const [filters, setFilters] = useState<InventoryCountFilters>(INITIAL_FILTERS);
  ...
  useEffect(() => {
    ...
    getInventoryCounts(token, filters, controller.signal)
      .then((data) => {
  ...
  }, [authStatus, filters, hasInventoryAccess, logout, router, token]);
  ...
  async function handleRetry(): Promise<void> {
    ...
    try {
      const data = await getInventoryCounts(token, filters);
  ...
  function updateFilters(
    updater: (current: InventoryCountFilters) => InventoryCountFilters,
  ): void {
    setLoadState({ status: "loading", data: null, message: null });
  ...
  function openInventoryCount(inventoryCountId: number): void {
    router.push(`/inventory/counts/${inventoryCountId}`);
  }
  ...
  function handleRowKeyDown(
    ...
    if (event.key === "Enter") {
      ...
      if (inventoryCount) {
        openInventoryCount(inventoryCount.id);
      }
  ...
  if (authStatus === "authenticated" && user && !hasInventoryAccess) {
    return (
      ...
      >
        <StatePanel
          title="No tiene acceso a conteos físicos"
  ...
  return (
    <AppShell
      ...
      description="Consulte conteos de inventario registrados y su estado."
      actions={
        hasWriteAccess ? (
          <Button
            type="button"
            onClick={() => {
              router.push("/inventory/counts/new");
            }}
          >
            Nuevo conteo
          </Button>
    ...
    >
      ...
      >
        <div className="border-b border-[var(--color-border-soft)] p-5 sm:p-6">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm leading-6 text-muted-foreground">
                Busque por referencia del conteo.
              </p>
            </div>
            ...
            >
              <div className="relative min-w-0 flex-1">
                ...
                <Input
                  ...
                  placeholder="Referencia del conteo"
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            <select
              ...
              onChange={(event) => {
                const status = event.target.value as InventoryCountFilters["status"];
            ...
            <select
              ...
              onChange={(event) => {
                const activeState =
                  event.target.value as InventoryCountFilters["activeState"];

                updateFilters((current) => ({ ...current, activeState, page: 1 }));
              }}
              className="h-11 rounded-[var(--radius-md)] border border-border bg-surface px-3 text-sm font-medium text-foreground shadow-sm focus:border-primary focus:outline-none focus:ring-4 focus:ring-[rgb(7_81_132_/_12%)]"
              aria-label="Filtrar conteos por estado activo"
            >
        ...
        {loadState.status === "loading" && (
          <LoadingState message="Consultando conteos…" />
        )}
        ...
        {loadState.status === "success" && loadState.data.results.length === 0 && (
          <div className="p-6">
            <StatePanel
              title={filters.query ? "No se encontraron conteos" : "No hay conteos registrados"}
              message={
                filters.query
                  ? "Revise la referencia o intente con otro filtro."
                  : "El sistema todavía no contiene conteos para el filtro seleccionado."
              }
          ...
          <>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[720px] border-collapse">
                ...
                <tbody>
                  {loadState.data.results.map((inventoryCount, index) => (
                    <tr
                      ...
                      tabIndex={0}
                      role="link"
                      aria-label={`Abrir conteo ${inventoryCount.reference}`}
                      onClick={() => {
                        openInventoryCount(inventoryCount.id);
                      }}
                      onKeyDown={(event) => {
                    ...
                    >
                      <td className="px-5 py-4 align-top">
                        <p className="font-semibold text-foreground">{inventoryCount.reference}</p>
                      </td>
                      ...
                      <td className="px-5 py-4 align-top">
                        <span
                          className={[
                            ...
                            STATUS_BADGE_CLASSES[inventoryCount.status],
                          ].join(" ")}
                        >
                          {STATUS_LABELS[inventoryCount.status]}
                        </span>
                      </td>
                    </tr>
                  ))}
