...
import { SearchIcon, TruckIcon } from "@/components/icons/app-icons";
...
function getLoadErrorMessage(error: unknown): string {
  ...
  return "No fue posible consultar los proveedores.";
}
...
export default function SuppliersPage() {
  ...
  useEffect(() => {
    ...
    getSuppliers(token, filters, controller.signal)
      .then((data) => {
  ...
  async function handleRetry(): Promise<void> {
    ...
    try {
      const data = await getSuppliers(token, filters);
  ...
  function updateFilters(
    updater: (current: SupplierFilters) => SupplierFilters,
  ): void {
    setLoadState({ status: "loading", data: null, message: null });
  ...
  function openSupplier(supplierId: number): void {
  ...
  return (
    <AppShell
      title="Proveedores"
      description="Consulte empresas proveedoras y sus productos asociados."
      actions={
        hasWriteAccess ? (
          <Button
            type="button"
    ...
    >
      ...
      >
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm leading-6 text-muted-foreground">
                Busque por nombre, contacto, teléfono, correo o país.
              </p>
            </div>
            ...
            >
              ...
              <select
                ...
                onChange={(event) => {
                  const activeState =
                    event.target.value as SupplierFilters["activeState"];
        ...
        {loadState.status === "loading" && (
          <LoadingState message="Consultando proveedores…" />
        )}
        ...
        {loadState.status === "success" && loadState.data.results.length === 0 && (
          <div className="p-6">
            <StatePanel
              title={filters.query ? "No se encontraron proveedores" : "No hay proveedores registrados"}
              message={
                filters.query
                  ? "Revise el nombre o intente con otro contacto, teléfono o país."
                  : "El catálogo todavía no contiene proveedores para el filtro seleccionado."
              }
              icon={<TruckIcon />}
              action={
                filters.query ? (
          ...
          <>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[820px] border-collapse">
                ...
                <tbody>
                  {loadState.data.results.map((supplier, index) => (
                    <tr
                      key={supplier.id}
                      ref={(element) => {
                      ...
                      tabIndex={0}
                      role="link"
                      aria-label={`Abrir proveedor ${supplier.name}`}
                      onClick={() => {
                        openSupplier(supplier.id);
                      }}
                      onKeyDown={(event) => {
