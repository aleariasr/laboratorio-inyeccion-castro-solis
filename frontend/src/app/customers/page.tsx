...
import { SearchIcon, UsersIcon } from "@/components/icons/app-icons";
...
import type { Customer, CustomerFilters } from "@/features/customers/types";
...
export default function CustomersPage() {
  ...
  useEffect(() => {
    ...
    getCustomers(token, filters, controller.signal)
      .then((data) => {
  ...
  function updateFilters(
    updater: (current: CustomerFilters) => CustomerFilters,
  ): void {
    setLoadState({ status: "loading", data: null, message: null });
  ...
  function openCustomer(customerId: number): void {
  ...
  if (authStatus === "authenticated" && user && !hasCustomersAccess) {
    return (
      ...
      >
        <StatePanel
          title="No tiene acceso a clientes"
  ...
  return (
    <AppShell
      title="Clientes"
      description="Consulte clientes registrados para ventas y servicios."
      actions={
        hasWriteAccess ? (
          <Button
            type="button"
    ...
    >
      ...
      >
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm leading-6 text-muted-foreground">
                Busque por nombre, identificación, teléfono o correo.
              </p>
            </div>
            ...
            >
              ...
              <select
                ...
                onChange={(event) => {
                  const customerType =
                    event.target.value as CustomerFilters["customerType"];

                  updateFilters((current) => ({ ...current, customerType, page: 1 }));
                }}
              ...
              <select
                ...
                onChange={(event) => {
                  const activeState =
                    event.target.value as CustomerFilters["activeState"];
        ...
        {loadState.status === "loading" && (
          <LoadingState message="Consultando clientes…" />
        )}
        ...
        {loadState.status === "success" && loadState.data.results.length === 0 && (
          <div className="p-6">
            <StatePanel
              title={filters.query ? "No se encontraron clientes" : "No hay clientes registrados"}
              message={
                filters.query
                  ? "Revise el nombre o intente con otra identificación, teléfono o correo."
                  : "El catálogo todavía no contiene clientes para el filtro seleccionado."
              }
              icon={<UsersIcon />}
              action={
                filters.query ? (
          ...
          <>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[820px] border-collapse">
                ...
                <tbody>
                  {loadState.data.results.map((customer, index) => (
                    <tr
                      key={customer.id}
                      ref={(element) => {
                      ...
                      tabIndex={0}
                      role="link"
                      aria-label={`Abrir cliente ${customer.display_name}`}
                      onClick={() => {
                        openCustomer(customer.id);
                      }}
                      onKeyDown={(event) => {
                    ...
                    >
                      <td className="px-5 py-4 align-top">
                        <p className="font-semibold text-foreground">{customer.display_name}</p>
                      </td>
